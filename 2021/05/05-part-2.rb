...
x = input.flatten(1).minmax do |a, b|
  a[0] <=> b[0]
end
x_range = x[0][0]..x[1][0]
y = input.flatten(1).minmax do |a, b|
  a[1] <=> b[1]
end
y_range = y[0][1]..y[1][1]
...
input.each do |line|
  ...
  elsif line[0][0] <= line[1][0] && line[0][1] <= line[1][1]
    y = line[0][1]
    for x in line[0][0]..line[1][0]
      arr[x][y] += 1
      y += 1
    end
  elsif line[1][0] <= line[0][0] && line[1][1] <= line[0][1]
    y = line[1][1]
    for x in line[1][0]..line[0][0]
      arr[x][y] += 1
      y += 1
    end
  elsif line[0][0] <= line[1][0] && line[1][1] <= line[0][1]
    y = line[0][1]
    for x in line[0][0]..line[1][0]
      arr[x][y] += 1
      y -= 1
    end
  elsif line[1][0] <= line[0][0] && line[0][1] <= line[1][1]
    y = line[1][1]
    for x in line[1][0]..line[0][0]
      arr[x][y] += 1
      y -= 1
